...
@pytest.mark.browser
def test_rpa_reports_even_on_failure_with_reraise(base_url, tmp_path) -> None:  # type: ignore[no-untyped-def]
    """reraise=True surfaces the VisusWebError; the report is written regardless."""
    out = tmp_path / "out"
    with pytest.raises(errors.VisusWebError):
        with rpa("t", headless=True, outdir=str(out), summary=False, reraise=True) as page:
            page.goto(f"{base_url}/forms.html")
...
@pytest.mark.browser
def test_rpa_clean_exit_on_failure(base_url, tmp_path) -> None:  # type: ignore[no-untyped-def]
    """By default a failed step exits cleanly (code 1, no traceback) after reporting."""
    out = tmp_path / "out"
