...
def test_options_headless_adds_flag(tmp_path):
    opts = chrome.build_options(headless=True, download_dir=str(tmp_path), user_data_dir=str(tmp_path / "p"))
    args = opts.arguments
...
def test_options_headed_omits_headless(tmp_path):
    opts = chrome.build_options(headless=False, download_dir=str(tmp_path), user_data_dir=str(tmp_path / "p"))
